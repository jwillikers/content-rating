"""
This file contains the Text class to contain data on the
classification and rating of a given text.
"""
...
class Text:
    ...
    def __init__(self, text_sentences):
        ...
        :param text_sentences: the sentences contained within the text,
         a list of Sentence objects.
        """

        self.title = ''     # Title of content
        self.creator = ''   # Creator/Author of content
        self.content_type = 4  # Type of content being rating, default to text
        self.offensive_sentences = dict()       # Keys are sentence indices, values are lists of
                                                # sentences's offensive categories
        self.sentence_list = text_sentences     # List of sentences in the text
        self.total_strongly_offensive_words_dict = dict()   # Keys are category names, values are dictionaries
                                                            # with offensive words as keys and counts as values.
        self.total_weakly_offensive_words_dict = dict()     # Keys are category name, values are dictionaries
                                                            # with offensive words as keys and counts as values
    ...
    def add_weakly_offensive_words(self, offensive_words: dict):
        ...
        from a single sentence to update the text's total count.
        :param offensive_words: A dictionary where the key contains the
        weakly offensive word and its category
    ...
    def add_offensive_words(self, number_of_offensive_words: int):
        ...
        increase the total number of offensive words
        by.
        :return: None.
        """
    ...
    def _calculate_offensive_ratio(self, numerator: int):
        """
    ...
    def _generate_category_ratings(self, user: User):
        ...
        for category in model_helper.get_categories():
            ...
            ratio = self._calculate_offensive_ratio(strongly_offensive_category_words)
    ...
    def generate_ratings(self, user: User):
        """
        This function generates an offensiveness rating using
        the text's classification data.
        :param user: An User
        :return: None.
        """
        # Category ratings must be done first since overall rating uses these values
